...
set xtics  font ",".xticfont nomirror \
  ...
  ("" 0, "2" 2, "" 4, "6" 6, "" 8, "10" 10, "" 12, "14" 14) # (0, 2, 4, 6, 8, 10, 12, 14)
#set xrange [-20:220];
#set xtics format "%g" rotate 90 font ",".xticfont 20
set xrange [0:14.9] # [0:14.9];
set yrange [0:*];
...
arr_yhi    =  "5000 220 110 65 110 110"
...
do for [z=1:12] {
    ...
    if (k == 1) {
# latency
	    plot sprintf("%s/result.lat.nic.csv", workload) \
	       u (cores($9)):(trexLat($16)) ls 4 t 'nmNFV-', \
                 sprintf("%s/result.lat.nic-inline.csv", workload) \
	       u (cores($9)):(trexLat($16)) ls 5 t 'nmNFV', \
		 sprintf("%s/result.lat.host.csv", workload) \
	       u (cores($9)):(trexLat($16)) ls 6 t 'split', \
		 sprintf("%s/result.lat.base.csv", workload) \
	       u (cores($9)):(trexLat($16)) ls 7 t 'host'
    }
    if (k == 2) {
# handled load
	    plot sprintf("%s/result.lat.nic.csv", workload) \
	       u (cores($9)):(trexRx($19)) ls 4 t 'nmNFV-', \
     sprintf("%s/result.lat.nic-inline.csv", workload) \
	       u (cores($9)):(trexRx($19)) ls 5 t 'nmNFV', \
		 sprintf("%s/result.lat.host.csv", workload) \
	       u (cores($9)):(trexRx($19)) ls 6 t 'split', \
		 sprintf("%s/result.lat.base.csv", workload) \
	       u (cores($9)):(trexRx($19)) ls 7 t 'host'
    }
    if (k == 3) {
# PCIe hit rate
	    plot sprintf("%s/result.lat.nic.csv", workload) \
	       u (cores($9)):(pcieUti($27)) ls 4 t 'nmNFV-', \
                sprintf("%s/result.lat.nic-inline.csv", workload) \
	       u (cores($9)):(pcieUti($27)) ls 5 t 'nmNFV', \
		 sprintf("%s/result.lat.host.csv", workload) \
	       u (cores($9)):(pcieUti($27)) ls 6 t 'split', \
		 sprintf("%s/result.lat.base.csv", workload) \
	       u (cores($9)):(pcieUti($27)) ls 7 t 'host'
    }
    if (k == 4) {
# memory bandwidth
	    plot sprintf("%s/result.lat.nic.csv", workload) \
	       u (cores($9)):4 ls 4 t 'nmNFV-', \
                 sprintf("%s/result.lat.nic-inline.csv", workload) \
	       u (cores($9)):4 ls 5 t 'nmNFV', \
		 sprintf("%s/result.lat.host.csv", workload) \
	       u (cores($9)):4 ls 6 t 'split', \
		 sprintf("%s/result.lat.base.csv", workload) \
	       u (cores($9)):4 ls 7 t 'host' 
    }
    if (k == 5) {
 # cache hit rate
 	    plot sprintf("%s/result.lat.nic.csv", workload) \
 	       u (cores($9)):(cacheHits($23,$24)) ls 4 t 'nmNFV-', \
                 sprintf("%s/result.lat.nic-inline.csv", workload) \
 	       u (cores($9)):(cacheHits($23,$24)) ls 5 t 'nmNFV', \
 		 sprintf("%s/result.lat.host.csv", workload) \
 	       u (cores($9)):(cacheHits($23,$24)) ls 6 t 'split', \
 		 sprintf("%s/result.lat.base.csv", workload) \
 	       u (cores($9)):(cacheHits($23,$24)) ls 7 t 'host'
     }
    if (k == 6) {
# PCIe hit rate
	    plot sprintf("%s/result.lat.nic.csv", workload) \
	       u (cores($9)):(hit_rate($5,$6)) ls 4 t 'nmNFV-', \
                sprintf("%s/result.lat.nic-inline.csv", workload) \
	       u (cores($9)):(hit_rate($5,$6)) ls 5 t 'nmNFV', \
		 sprintf("%s/result.lat.host.csv", workload) \
	       u (cores($9)):(hit_rate($5,$6)) ls 6 t 'split', \
		 sprintf("%s/result.lat.base.csv", workload) \
	       u (cores($9)):(hit_rate($5,$6)) ls 7 t 'host'
    }
